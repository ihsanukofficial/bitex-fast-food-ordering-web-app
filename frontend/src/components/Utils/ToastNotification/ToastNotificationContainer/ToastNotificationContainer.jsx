...
import { ACCOUNT_TOAST_EVENT, ORDER_NOTIFICATION_TOAST_EVENT } from '../../../../utils/notificationConstants';
...
/**
 * Converts a cart mutation result or a real-time order notification into concise
...
function ToastNotificationContainer() {
  ...
  useEffect(() => {
    ...
    const handleCartItemAddResult = (event) => pushNotification(event.detail || {});
    const handleOrderNotification = (event) => pushNotification(event.detail || {});
    const handleAccountToast = (event) => pushNotification(event.detail || {});
    ...
    window.addEventListener(
      ORDER_NOTIFICATION_TOAST_EVENT,
      handleOrderNotification,
    );
    window.addEventListener(ACCOUNT_TOAST_EVENT, handleAccountToast);
    ...
    return () => {
      ...
      window.removeEventListener(ACCOUNT_TOAST_EVENT, handleAccountToast);
